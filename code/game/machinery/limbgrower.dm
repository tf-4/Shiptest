...
/obj/machinery/limbgrower/ui_static_data(mob/user)
	...
	for(var/design_id in stored_research.unlocked_designs)
		var/datum/design/limb_design = stored_research.unlocked_designs[design_id]
...
/obj/machinery/limbgrower/ui_act(action, list/params)
	...
	switch(action)
		...
		if("make_limb")
			being_built = stored_research.unlocked_designs[params["design_id"]]
			if(!being_built)
